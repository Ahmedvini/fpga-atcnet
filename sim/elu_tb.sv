// =============================================================================
// elu_tb.sv  —  Bit-exact regression for the ELU LUT module
//
// Streams the Branch A depthwise output (T=600 × F2=32) into elu.sv and
// compares every output vector against the Python LUT reference.
//
// Setup:
//   python3 scripts/gen_elu_lut.py
//   python3 scripts/q88_layer5.py
// =============================================================================
...
module elu_tb;
    ...
    localparam int LUT_RANGE_Q = 2048;
    localparam int LUT_SHIFT   = 3;
    localparam int CLK_PERIOD  = 10;

    logic signed [DATA_WIDTH-1:0] in_mem  [0:T*NUM_CH - 1];
    logic signed [DATA_WIDTH-1:0] exp_mem [0:T*NUM_CH - 1];

    initial begin
        $readmemh("data/golden_q88/stage_branchA_elu_input.hex",  in_mem);
        $readmemh("data/golden_q88/stage_branchA_elu_output.hex", exp_mem);
    end
                ...
                for (int c = 0; c < NUM_CH; c++) begin
                    ...
                    if (y_out[c] !== exp_v) begin
                        if (mismatches < 8) begin
                            $display("[elu_tb] mismatch t=%0d c=%0d  got=%h  exp=%h",
    ...
    initial begin
        ...
        $display("[elu_tb] vectors driven  : %0d (expected %0d)", n_drive, T);
        $display("[elu_tb] outputs received: %0d (expected %0d)", outputs_received, T);
        $display("[elu_tb] mismatches      : %0d", mismatches);
        if (outputs_received != T)         $fatal(1, "output count off");
        if (mismatches != 0) begin
            $display("[elu_tb] first mismatch t=%0d c=%0d got=%h exp=%h",
